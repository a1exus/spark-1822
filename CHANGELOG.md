...
- `llama-cpp/.env.example`: switch `LLAMACPP_TAG` from a digest-pinned floating `server-cuda@sha256:…` to the immutable per-build tag `server-cuda-b5343` (the latest `server-cuda-b<NNNN>` actually published to `ghcr.io/ggml-org/llama.cpp`). Per-build tags are immutable on the registry, so the digest-pin was redundant. README rewritten to explain the per-build tag approach (with digest-pinning kept as an optional belt-and-suspenders step). `LLAMACPP_TAG_DEFAULT` removed from the Makefile — dead variable after the host/variant config split.
