...
- Every Traefik router's `rule=` switched from a hardcoded `Host()` literal to a single `HostRegexp(`<svc>.spark{x:.+}`)` matcher. Matches the per-service subdomain form across any LAN/tailnet/Cloudflare domain (`vllm.spark-1822.local`, `vllm.spark-1822.<tailnet>.ts.net`, `vllm.sparky.example.com`, …). Six routers updated: `ollama`, `open-webui`, `vllm`, `llama` (label-based, in each app's compose) and `netdata`, `traefik` (file-based, in `traefik/dynamic/services.yml`). Renames of the hostname (`spark-1822` → anything starting with `spark`) or the LAN/tailnet/public domain no longer require touching any rule. A short-lived second matcher `HostRegexp(`spark{x:.+}`)` was tried (so the bare tailnet hostname `spark-1822.<tailnet>.ts.net` would also land on a router via rule-length tiebreaker → Open WebUI) but dropped: the implicit tiebreaker behaviour was confusing, and the right pattern is per-service tailnet hostnames via Tailscale HTTPS subdomains. `tailscale/README.md` "Host-header routing" and `traefik/README.md` "Add an app" sections track the final state.
