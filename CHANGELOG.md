...
- Every Traefik router's `rule=` switched from a hardcoded pair of `Host()` literals to a pair of `HostRegexp` matchers: `HostRegexp(`<svc>.spark{x:.+}`) || HostRegexp(`spark{x:.+}`)`. The first matches the per-service subdomain form (`vllm.spark-1822.local`, `ollama.sparky.example.com`, …); the second matches the bare-host form Tailscale Serve / Cloudflare Tunnel / a renamed LAN host can present (`spark-1822.cuscus-macaroni.ts.net`, etc.). Six routers updated: `ollama`, `open-webui`, `vllm`, `llama` (label-based, in each app's compose) and `netdata`, `traefik` (file-based, in `traefik/dynamic/services.yml`). Renames of the hostname (`spark-1822` → anything starting with `spark`) or the LAN/tailnet domain no longer require touching any rule. Because all six routers' second matcher fires on the same bare tailnet hostname, Traefik resolves the conflict by default rule-length priority — `open-webui` wins, so `https://spark-1822.cuscus-macaroni.ts.net/` lands on Open WebUI; other services stay reachable on their per-service URLs. `tailscale/README.md` "Host-header routing" and `traefik/README.md` "Add an app" sections updated to match.
