...
- `llama-cpp/.env.example`: switch `LLAMACPP_TAG` from a digest-pinned floating `server-cuda@sha256:…` to the immutable per-build tag `server-cuda-b9151`. `ggml-org/llama.cpp` publishes a `server-cuda-b<NNNN>` tag for every upstream commit; those tags are immutable on the registry, so the digest-pin was redundant. README rewritten to explain the per-build tag approach (with digest-pinning kept as an optional belt-and-suspenders step). `LLAMACPP_TAG_DEFAULT` removed from the Makefile — dead variable after the host/variant config split.
